...
export const useComicStore = defineStore("comicStore", {
  ...
  actions: {
    async getComics() {
      this.loading = true;
      const all = await fetch("https://second-backend-web-service-0qh4.onrender.com/api/manhwaLab/manhwaList");
    ...
    async getTopComics() {
      const daily = await fetch(
        "https://second-backend-web-service-0qh4.onrender.com/api/manhwaLab/top/daily"
      );
      const dailyJson = await daily.json();

      const weekly = await fetch(
        "https://second-backend-web-service-0qh4.onrender.com/api/manhwaLab/top/weekly"
      );
      const weeklyJson = await weekly.json();

      const monthly = await fetch(
        "https://second-backend-web-service-0qh4.onrender.com/api/manhwaLab/top/monthly"
      );
    ...
    increaseViewCount(id) {
      fetch(`https://second-backend-web-service-0qh4.onrender.com/api/manhwaLab/${id}/update-view`, {
        method: "POST",
        headers: {
